...
void	setsegs()
{
	...
	ptss = &gdt_copy[7] ; /* TSS segment */
	ptss->sd_lolimit = 0xffff & ( (uint32) sizeof( global_tss ) - 1 ) ;
	ptss->sd_lobase = 0xffff & ( (uint32) &global_tss ) ;
	ptss->sd_midbase = 0xff & ( ( (uint32) &global_tss ) >> 16 ) ;
	ptss->sd_access = 0x89 ;
	ptss->sd_hilim_fl = 0x00 ;
	ptss->sd_hibase = 0xff & ( ( (uint32) &global_tss ) >> 24 ) ;
	
	global_tss.ss0 = ( 0x3 << 3 ) ; /* Kernel stack segment */
